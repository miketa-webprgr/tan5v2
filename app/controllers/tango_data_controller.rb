class TangoDataController < Base
  ...
  def change
    tango_datum = @current_user.tango_datum.find_by(tango_id: params[:tango_id])
    if tango_datum == nil
      tango_datum = @current_user.tango_datum.build(tango_id: params[:tango_id], wordnote_id: params[:wordnote_id],wrong_num: 0, trial_num: 0, star: 0)
    end
  ...
  def get_tango_data
    ...
    @correct_ratio = 0
    @star = nil
    p tango_datum = @current_user.tango_datum.find_by(tango_id: params[:tango_id])
    if tango_datum
